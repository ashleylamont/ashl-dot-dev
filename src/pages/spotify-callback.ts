...
export async function GET({cookies, request}: APIContext) {
  ...
  const {access_token, expires_in, refresh_token} = await fetch("https://accounts.spotify.com/api/token?"+querystring.stringify({
    code,
    ...
    method: "POST",
    headers: {
      "Authorization": `Basic ${new Buffer.from(`${import.meta.env.SPOTIFY_CLIENT_ID}:${import.meta.env.SPOTIFY_CLIENT_SECRET}`).toString('base64')}`,
  ...
  if (userProfile.id !== import.meta.env.SPOTIFY_USER_ID) {
